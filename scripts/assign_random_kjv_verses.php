...
$rows = Verse::where('verse_english', 'like', 'TBD verse%')->get();

if ($rows->isEmpty()) {
    echo "No placeholder verses found to update.\n";
    exit;
}
...
foreach ($rows as $row) {
    ...
    // For Tamil, set a simple placeholder indicating the verse reference (KJV reference at start)
    $ref = preg_match('/^[A-Za-z0-9 ]+:\d+/', $rand, $matches) ? $matches[0] : '';
    $tamil = ($ref ? "தமிழ்: " . $ref . " (தமிழ் மொழிபெயர்ப்பு)" : "தமிழ்: (தமிழ் மொழிபெயர்ப்பு)");

    $row->verse_english = $rand;
    $row->verse_tamil = $tamil;
    $row->save();
    $updated++;
    echo "Updated verse id {$row->verse_id} (date: {$row->week_date})\n";
}
